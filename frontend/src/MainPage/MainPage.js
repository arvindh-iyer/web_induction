...
import { AppBar, Button, Stack, Switch, ThemeProvider, Typography } from '@mui/material'
...
export const MainPage=()=>{
  ...
  useEffect(()=>{
    (async()=>{
        let name=await fetch('http://localhost:5000/mainpage/verifyJwt',{
            method:'post',
  ...
  return(
      ...
      <div>
        ...
        <CustomAppBar sx={{display:'flex',flexDirection:'row',alignItems:'center',position:'relative'}}>
            <Grid container sx={{padding:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
